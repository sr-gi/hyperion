    ...
    pub fn process_scheduled_announcement_outbounds(
        ...
        if peer.is_erlay() {
            ...
            if to_be_announced {
                ...
                let should_fanout = self.received_invs + self.out_fanout_count
                    <= *OUTBOUND_FANOUT_THRESHOLD
                    || self.out_fanout_count < 4;
                ...
                if should_fanout {
                    ...
                    peer_mut.get_tx_reconciliation_state_mut().unwrap().add_tx();
                }
            }
            ...
            for (peer_id, peer) in self.get_inbounds_mut().iter_mut() {
                ...
                if peer.to_be_announced() {
                    ...
                    if in_fanout_count < inbound_target_count {
                        ...
                        peer.get_tx_reconciliation_state_mut().unwrap().add_tx();
                    }
                }
