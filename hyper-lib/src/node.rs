...
impl Node {
    ...
    fn schedule_tx_announcement(
        ...
        let mut inbound_fanout_targets = self.get_fanout_targets(
        ...
        inbound_fanout_targets.shuffle(&mut self.rng);
        ...
        for peer_id in peers {
            ...
            if !self.get_peer(&peer_id).unwrap().knows_transaction() {
                let fanout_targets = if self.is_peer_inbounds(&peer_id) {
                    &inbound_fanout_targets
                } else {
                    &outbound_fanout_targets
                };
...
mod test_node {
    ...
    fn test_schedule_tx_announcement_erlay() {
        ...
        let outbound_fanout_targets = node.get_fanout_targets(
            node.out_peers.keys().copied().collect::<Vec<_>>(),
            OUTBOUND_FANOUT_DESTINATIONS as f64,
        );
