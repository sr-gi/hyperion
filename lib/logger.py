...
class log(object):
    ...
    @staticmethod
    def deserialize_part(serialization):
    ...
    def __str__(self):
        return "[{0}] ({1}) {2}: {3}, {4}".format(unix2str(self.timestamp), self.source_id,
                                                  log_types.str_mapping[self.log_type], self.rest, self.timestamp)
...
class bitcoin_log(log):  # bitcoin connection/disconnection events
    def repack(self):
        ...
        second = pack('=hHI8xhHI8x', socket.AF_INET, self.r_port_, self.r_addr_, socket.AF_INET, self.l_port_,
                      self.l_addr_)
        third = pack('>Is', len(self.text), self.text)
        return first + second + third

    def __init__(self, source_id, timestamp, handle_id, update_type, remote_addr, remote_port, local_addr, local_port,
                 text):
    ...
    def __str__(self):
        return "[{0}] ({1}) {2}: handle: {3} update_type: {4}, remote: {5}:{6}, local: {7}:{8}, text: {9} {10}".format(
            unix2str(self.timestamp), self.source_id, log_types.str_mapping[self.log_type], self.handle_id,
            update_types.str_mapping[self.update_type], self.remote_addr, self.remote_port, self.local_addr,
            self.local_port, self.text, self.timestamp)
...
class bitcoin_msg_log(log):
    ...
    def __str__(self):
        return "[{0}] ({1}) {2}: handle_id: {3}, is_sender: {4}, bitcoin_msg: (ommitted), {5}".format(
            unix2str(self.timestamp), self.source_id, log_types.str_mapping[self.log_type], self.handle_id,
            self.is_sender, self.timestamp)
