...
class message(object):  # Just a generic message
    ...
    def deserialize(serialization):
        ...
        if version != 0 or length != len(payload):
            # ToDo: Check this!
            raise Exception()   # Use to be Exception(res) with res undefined
        return message(message_type, payload)


class register_msg(message):  # Re-registers the client connection. Use this to garbage collect all registered messages
    def __init__(self):
        super(register_msg, self).__init__(message_types.REGISTER, '')
...
class connect_msg(message):
    def repack(self):
        return pack('=hHI8xhHI8x', socket.AF_INET, self.r_port_, self.r_addr_, socket.AF_INET, self.r_port_,
                    self.r_addr_)
...
class command_msg(message):
    def repack(self):
        if len(self.targets_) > 0:
            ...
            tl = self.targets_
            return pack(packstr, self.command_, int(self.message_id_), len(self.targets_), *tl)
        else:
...
class connection_info(object):
    ...
    def repack(self):
        return pack('>I', self.handle_id) + pack('=hHI8xhHI8x', socket.AF_INET, self.r_port_, self.r_addr_,
                                                 socket.AF_INET, self.r_port_, self.r_addr_)
